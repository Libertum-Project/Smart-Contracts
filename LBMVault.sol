...
import "./IBEP20.sol";
import "./LBMVaultProxy.sol";
import "./LBMVaultStorage.sol";
import "./LBMVaultErrorReporter.sol";
...
contract LBMVault is LBMVaultStorage, ECDSA {
    ...
    event StoreUpdated(address oldLbm, address oldStore, address newLbm, address newStore);
    ...
    function add(
        ...
        onlyAdmin
    {
        require(address(lbmStore) != address(0), "Store contract addres is empty");
        ...
        ILBMStore(lbmStore).setRewardToken(_rewardToken, true);
    ...
    /**
     * @notice Deposit LBMVault for LBM allocation
        ...
        nonReentrant
    {
        ...
        if (user.amount > 0) {
            ...
            ILBMStore(lbmStore).safeRewardTransfer(_rewardToken, msg.sender, pending);
        }
        ...
        if (address(pool.token) == address(lbmAddress)) {
    ...
    /**
     * @notice Execute withdrawal to LBMVault for LBM allocation
     * @param _rewardToken The Reward Token Address
     * @param _pid The Pool Index
     */
    ...
    /**
     * @notice Request withdrawal to LBMVault for LBM allocation
     * @param _rewardToken The Reward Token Address
     * @param _pid The Pool Index
        ...
        nonReentrant
    {
        ...
        // Update Delegate Amount
        if (_rewardToken == address(lbmAddress)) {
    ...
    // View function to see pending LBMs on frontend.
    ...
    /**
     * @notice Get the LBM stake balance of an account (excluding the pending withdrawals)
     * @param account The address of the account to check
     * @return The balance that user staked
     */
    function getStakeAmount(address account) internal view returns (uint96) {
        require(lbmAddress != address(0), "LBMVault::getStakeAmount: lbm address is not set");

        PoolInfo[] storage poolInfo = poolInfos[lbmAddress];

        uint256 length = poolInfo.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            if (address(poolInfo[pid].token) == address(lbmAddress)) {
                UserInfo storage user = userInfos[lbmAddress][pid][account];
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s)
        external
    {
        bytes32 domainSeparator = keccak256(abi.encode(DOMAIN_TYPEHASH, keccak256(bytes("LBMVault")), getChainId(), address(this)));
        bytes32 structHash = keccak256(abi.encode(DELEGATION_TYPEHASH, delegatee, nonce, expiry));
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", domainSeparator, structHash));
        address signatory = ECDSA.recover(digest, v, r, s);
        require(nonce == nonces[signatory]++, "LBMVault::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "LBMVault::delegateBySig: signature expired");
    ...
    function _moveDelegates(address srcRep, address dstRep, uint96 amount) internal {
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint96 srcRepNew = sub96(srcRepOld, amount, "LBMVault::_moveVotes: vote amount underflows");
            ...
            if (dstRep != address(0)) {
                ...
                uint96 dstRepNew = add96(dstRepOld, amount, "LBMVault::_moveVotes: vote amount overflows");
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint96 oldVotes, uint96 newVotes) internal {
        uint32 blockNumber = safe32(block.number, "LBMVault::_writeCheckpoint: block number exceeds 32 bits");
    ...
    /**
     * @notice Determine the lbm stake balance for an account
     * @param account The address of the account to check
     * @param blockNumber The block number to get the vote balance at
     * @return The balance that user staked
     */
    function getPriorVotes(address account, uint256 blockNumber) external view returns (uint96) {
        require(blockNumber < block.number, "LBMVault::getPriorVotes: not yet determined");
    ...
    function _become(LBMVaultProxy lbmVaultProxy) external {
        require(msg.sender == lbmVaultProxy.admin(), "only proxy admin can change brains");
        require(lbmVaultProxy._acceptImplementation() == 0, "change not authorized");
    }

    function setLbmStore(address _lbm, address _lbmStore) external onlyAdmin {
        address oldLbmContract = lbmAddress;
        address oldStore = lbmStore;
        lbmAddress = _lbm;
        lbmStore = _lbmStore;

        _notEntered = true;

        emit StoreUpdated(oldLbmContract, oldStore, _lbm, _lbmStore);
    }
}
